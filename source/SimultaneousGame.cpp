...
public:
	...
	SimultaneousMazeState(const int h, const int w, const int seed) :
		h_(h),
		w_(w),
...
int main() {
	std::vector<StringAIPair> ais = {
		...
		StringAIPair("primitiveMontecarloAction",[&](const State& state,const int player_id) {return primitiveMontecarloAction(state,player_id,1000); }),
		StringAIPair("ductAction",[&](const State& state,const int player_id) {return ductAction(state,player_id,1000); }),
